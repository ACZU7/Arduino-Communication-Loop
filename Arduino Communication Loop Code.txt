...
const int X_pin = A1;   // Analog pin connected to X-axis (controls fan speed)
const int motorPin = 5; // PWM pin connected to motor control
...
void readInputs()
{
  ...
  if (digitalRead(buttonPin) == LOW){txButton =1;}
  else { txButton =0;}

  if (digitalRead(tiltSwitch) == LOW){txTilt =1;}
  else { txTilt =0;}
  ...
  int motorSpeed = 0;
  if (xValue >= 512) {
    motorSpeed = map(xValue, 512, 1023, 0, 127); // Map upper half of range
  } else {
    motorSpeed = map(xValue, 511, 0, 0, 127);    // Map lower half of range
  }
  txB = motorSpeed;
}
...
void writeOutputs()
{
  ...
  digitalWrite(ledTilt, rxTilt);
  



  analogWrite(motorPin, 2*(int)rxB);
